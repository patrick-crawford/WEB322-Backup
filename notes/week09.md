...
![AJAX Request Payload](/WEB322-Backup/media/uploads/2017/05/ajax-send2.png)

If we wish to capture this information in our routes (so that we can make the appropriate updates to our data source), we must make some small modifications to our server.js file and individual routes (ie: POST to "/api/users" & PUT to "/api/users/:userId"). The first thing that we must do is incorporate middleware to parse the incoming data. Recall from week 5 - we used "express.urlencoded()" access the "body" component of the request.  However, since we will be parsing JSON data, we must instead use "express.json()", ie:
